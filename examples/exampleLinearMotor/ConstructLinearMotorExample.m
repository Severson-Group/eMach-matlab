...
magnet3(i) = CrossSectSolidRect( ...
        'name', ...
        'dim_w', ['magnet' num2str(i)],DimMlimeter(d_rm),...
        'dim_h',DimMillimeter(w_rr),...
        'location', Location2D( ...
...
moverAirGap(i) = CrossSectNotchedRectangle( ...
        ...
        'location', Location2D( ...
            'anchor_xy', DimMillimeter([r_si-gap/2, w-w*(i-1)]), ...
            'theta', DimDegree([90]).toRadians() ...
        ) ...
        );
end
...
magnet1Comp(i) = Component( ...
        ...
        'material', MaterialGeneric('name', MagnetMaterial), ...
        'makeSolid', MakeRevolve( ...
            'location', Location3D( ...
                'anchor_xyz', DimMillimeter([0,0,0]), ...
                'rotate_xyz', DimDegree([0,0,0]).toRadians() ...
...
if (DRAW_MAGNET)
    ...
    % Create components
    for i = 1:NS/2
        statorIronComp(i).make(toolMN, toolMN);   
        statorAirGapComp(i).make(toolMN, toolMN);   
        slotOpening1Comp(i).make(toolMN, toolMN);
        slotOpening2Comp(i).make(toolMN, toolMN);
        coil11Comp(i).make(toolMN, toolMN);
        coil12Comp(i).make(toolMN, toolMN);
        coil21Comp(i).make(toolMN, toolMN);
        coil22Comp(i).make(toolMN, toolMN);
    end
    for i = 1:np
        moverIronComp(i).make(toolMN, toolMN);
        magnet1Comp(i).make(toolMN, toolMN);
        mn_d_setparameter(toolMn.doc, magnet1Comp.name, ...
            'MaterialDirection','[0, 1, 0]',...
            get(toolMn.consts,'InfoArrayParameter'));
        magnet2Comp(i).make(toolMN, toolMN);
        mn_d_setparameter(toolMn.doc, magnet1Comp.name, 'MaterialDirection','[0, 1, 0]',get(toolMn.consts,'InfoArrayParameter'));
        magnet3Comp(i).make(toolMN, toolMN);
        mn_d_setparameter(toolMn.doc, magnet1Comp.name, 'MaterialDirection','[0, 1, 0]',get(toolMn.consts,'InfoArrayParameter'));
        moverAirGapComp(i).make(toolMN, toolMN);
    end
    toolMn.viewAll();
...
%     sprintf('%g %%mm', MES_Remesh), ...
%     get(toolMn.consts,'infoNumberParameter'));
% mn_d_setparameter(toolMn.doc, compRotor1VA.name, 'MaximumElementSize', ...
%     sprintf('%g %%mm', MES_RotorVA), ...
%     get(toolMn.consts,'infoNumberParameter'));
% mn_d_setparameter(toolMn.doc, compRotor2VA.name, 'MaximumElementSize', ...
%     sprintf('%g %%mm', MES_RotorVA), ...
%     get(toolMn.consts,'infoNumberParameter'));
% mn_d_setparameter(toolMn.doc, compStatorVA.name, 'MaximumElementSize', ...
%     sprintf('%g %%mm', MES_StatorVA), ...
%     get(toolMn.consts,'infoNumberParameter'));
% mn_d_setparameter(toolMn.doc, compMag1s.name, 'MaximumElementSize', ...
%     sprintf('%g %%mm', MES_Magnets), ...
%     get(toolMn.consts,'infoNumberParameter'));
% for i=1:2*p-1
% mn_d_setparameter(toolMn.doc, compMag1(i).name, 'MaximumElementSize', ...
%     sprintf('%g %%mm', MES_Magnets), ...
%     get(toolMn.consts,'infoNumberParameter'));
% end
% mn_d_setparameter(toolMn.doc, compMag1e.name, 'MaximumElementSize', ...
%     sprintf('%g %%mm', MES_Magnets), ...
%     get(toolMn.consts,'infoNumberParameter'));
% mn_d_setparameter(toolMn.doc, compMag2s.name, 'MaximumElementSize', ...
%     sprintf('%g %%mm', MES_Magnets), ...
%     get(toolMn.consts,'infoNumberParameter'));
% for i=1:2*p-1
% mn_d_setparameter(toolMn.doc, compMag2(i).name, 'MaximumElementSize', ...
%     sprintf('%g %%mm', MES_Magnets), ...
%     get(toolMn.consts,'infoNumberParameter'));
% end
% mn_d_setparameter(toolMn.doc, compMag2e.name, 'MaximumElementSize', ...
%     sprintf('%g %%mm', MES_Magnets), ...
%     get(toolMn.consts,'infoNumberParameter'));
% 
...
% status = mn_d_setEvenPeriodic(toolMn.mn,bc_path,[], [], [], [], [-2*p*pole_pitch 0 0], []);
% 
% %%Set up Motion
% if type==2
%      motion = mn_d_makeMotionComponent(toolMn.mn,[{'compCoilIn1'}, {'compCoilIn2'},{'compCoilIn3'}...
%     {'compCoilOut1'},{'compCoilOut2'},{'compCoilOut3'},{'compCoilInIron1'}, {'compCoilInIron2'},{'compCoilInIron3'}...
%     {'compCoilOutIron1'},{'compCoilOutIron2'},{'compCoilOutIron3'},{'compStator2Remesh'},{'compStator1Remesh'},{'compStatorVA'}]);
%     PositionAtStartup = 0;
%     SpeedAtStartup = 0;
%     TimeArray = 0;
%     SpeedArray = linspeed;
...
if (DRAW_XFEMM)
    ...
    % Add new materials
    Magnet = newmaterial_mfemm(MagnetMaterial, 'Mu_x', 1.0997785, 'Mu_y', 1.0997785, 'H_c', 890000, 'Sigma', 0.625);
    load('M19_24Ga');
